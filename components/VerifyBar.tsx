...
export function VerifyBar({ onVerify, currentState, errorMessage, className }: VerifyBarProps) {
  ...
  const barVariants = {
    ...
    complete: { width: '100%', maxWidth: '48rem', height: '4.5rem', borderRadius: '1.5rem', borderColor: 'rgba(74, 222, 128, 0.4)' },
  ...
  const glowVariants = {
    ...
    complete: { opacity: 0.6, scale: 1, filter: 'blur(20px)', backgroundColor: 'rgba(74, 222, 128, 0.2)' },
  ...
  return (
    <div className={twMerge("relative flex flex-col items-center justify-center w-full", className)}>
      ...
      <motion.form
        ...
        className={clsx(
          "relative z-10 flex items-center overflow-hidden glass-panel backdrop-blur-2xl border transition-colors duration-300",
          {
            ...
            'border-green-500/50 bg-green-950/20': displayState === 'complete',
          }
        )}
      ...
      >
        <div className="flex items-center justify-center w-16 h-full text-foreground/50">
        ...
        <input
          ...
          className="flex-1 h-full bg-transparent border-none outline-none text-foreground placeholder:text-foreground/30 text-lg sm:text-xl font-medium disabled:opacity-50"
        />
        ...
        <AnimatePresence>
          {(displayState === 'input' || displayState === 'error') && query.length > 0 && (
            <motion.button
              ...
              exit={{ opacity: 0, x: 20 }}
              type="submit"
              className="px-6 h-full font-semibold text-sm tracking-wide bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 border-l border-glass-border transition-colors"
            >
              VERIFY
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {displayState === 'error' && errorMessage && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full left-0 right-0 flex items-center justify-center gap-2 text-red-400 text-sm font-medium"
          >
            <AlertCircle size={16} />
            {errorMessage}
          </motion.div>
